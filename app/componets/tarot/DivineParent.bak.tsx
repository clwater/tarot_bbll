...
export  async function  DivineParent({type: id = '0', randomId: randomId = '0'}) {
    ...
    function updateMatrix() {
        matrix.map((row: number[], rowIndex: number) => {
                row.map((element, columnIndex) => {
                    ...
                    matrixMap.set(rowIndex * 10 + columnIndex, matrix[rowIndex][columnIndex])
    ...
    function getTarot(id: number){
        const positionCardId = matrixMap.get(id)
        return tarots.find((tarot: TarotEntity) => tarot.index === positionCardId)
    }
